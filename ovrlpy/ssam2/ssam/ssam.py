...
import src.ssam2.ssam.utils as utils
...
def sample_expression(
    ...
    if mode == '2d':
        ...
        raise ValueError("Could not determine whether to use '2d' or '3d' analysis mode. Please specify mode='2d' or mode='3d'.") 
...
def _sample_expression_2d(
                        ...
                        x_column='x',y_column='y',gene_column='gene',
                        n_workers=8,
                        ):
    ...
    if coordinate_dataframe is None:
        if x is None or y is None or genes is None:
            raise ValueError("Either coordinate_dataframe_ or x,y,genes must be provided.")
        else:
    ...
    coordinate_dataframe_[[x_column,y_column]] /= kde_bandwidth/1.5 #lower resolution instead of increasing bandwidth!

    print("determining local maxima:")
    x_,_x = int(np.min(coordinate_dataframe_[x_column])),int(np.ceil(np.max(coordinate_dataframe_[x_column])))
    y_,_y = int(np.min(coordinate_dataframe_[y_column])),int(np.ceil(np.max(coordinate_dataframe_[y_column])))

    # perform a global KDE to determine local maxima:
    vector_field_norm = utils.kde_2d(coordinate_dataframe_[[x_column,y_column]].values,bandwidth=1.5)
    local_maximum_coordinates = utils.find_local_maxima(vector_field_norm,min_pixel_distance=min_pixel_distance,min_expression=minimum_expression)
    ...
    adata_ssam = anndata.AnnData(var=pd.DataFrame(index=gene_list),obs=pd.DataFrame(index=range(len(local_maximum_coordinates))))

    adata_ssam.obsm['spatial'] = local_maximum_coordinates*kde_bandwidth

    adata_ssam.uns['ssam'] = {'kde_bandwidth':kde_bandwidth,'x_':x_,'_x':_x,'y_':y_,'_y':_y,'size':size,
                                'coordinates':coordinate_dataframe, 'x_column':x_column,'y_column':y_column,'gene_column':gene_column}
    adata_ssam.X = np.zeros((len(local_maximum_coordinates),len(gene_list)))

    df_gene_grouped = coordinate_dataframe_.groupby(gene_column,observed=False).apply(lambda x: x[[x_column,y_column]].values)
    ...
    with ThreadPoolExecutor(max_workers=n_workers) as executor:
        fs = {executor.submit(utils.kde_and_sample,df_gene_grouped.loc[gene],local_maximum_coordinates,size=size):gene for gene in gene_list}
...
def _sample_expression_3d(
    ...
    if coordinate_dataframe is None:
        if x is None or y is None or z is None or genes is None:
            raise ValueError("Either coordinate_dataframe_ or x,y,z,genes must be provided.")
        else:
            coordinate_dataframe_ = pd.DataFrame({
                x_column:x,
                y_column:y,
    ...
    coordinate_dataframe_[[x_column,y_column,z_column]] /= kde_bandwidth #lower resolution instead of increasing bandwidth!

    print("determining local maxima:")
    x_,_x = int(np.min(coordinate_dataframe_[x_column])),int(np.ceil(np.max(coordinate_dataframe_[x_column])))
    y_,_y = int(np.min(coordinate_dataframe_[y_column])),int(np.ceil(np.max(coordinate_dataframe_[y_column])))
    z_,_z = int(np.min(coordinate_dataframe_[z_column])),int(np.ceil(np.max(coordinate_dataframe_[z_column])))

    print(x_,_x,y_,_y,z_,_z)
    # perform a global KDE to determine local maxima:
    vector_field_norm = utils.kde_3d(coordinate_dataframe_[[x_column,y_column,z_column]].values,bandwidth=1.1)
    local_maximum_coordinates = utils.find_local_maxima(vector_field_norm,min_pixel_distance=min_pixel_distance,min_expression=minimum_expression)
    ...
    adata_ssam.uns['ssam'] = {'kde_bandwidth':kde_bandwidth,'x_':x_,'_x':_x,'y_':y_,'_y':_y,'z_':z_,'_z':_z,'size':size,
                                'coordinates':coordinate_dataframe, 'x_column':x_column,'y_column':y_column,'z_column':z_column,'gene_column':gene_column}
    adata_ssam.X = np.zeros((len(local_maximum_coordinates),len(gene_list)))

    df_gene_grouped = coordinate_dataframe_.groupby(gene_column,observed=False).apply(lambda x: x[[x_column,y_column,z_column]].values)
...
def produce_map(adata, signatures=None, adata_obs_label='leiden', kernel_bandwidth=2.5, output_um_p_px=1,
         patch_length=1000, threshold_exp=0.1, threshold_cor=0.1, background_value=-1,
         fill_blobs=True, min_blob_area=10, n_workers=8):

    if signatures is None:
        signatures = pd.DataFrame(columns=adata.obs[adata_obs_label].unique(), index=adata.var.index) #celltype x gene
        for celltype in signatures.columns:
            signatures[celltype] = adata[adata.obs[adata_obs_label]==celltype].X.mean(0)

    kernel_bandwidth_px = kernel_bandwidth/output_um_p_px

    coordinate_df = adata.uns['ssam']['coordinates']
    x_column,y_column,gene_column = adata.uns['ssam']['x_column'],adata.uns['ssam']['y_column'],adata.uns['ssam']['gene_column']

    out_shape = (np.ceil(coordinate_df[x_column].max()/output_um_p_px+kernel_bandwidth_px*3).astype(int),
                 np.ceil(coordinate_df[y_column].max()/output_um_p_px+kernel_bandwidth_px*3).astype(int))

    ct_map = np.zeros((out_shape), dtype=int)+background_value
    vf_norm = np.zeros(ct_map.shape,dtype=float)

    patch_delimiters_x = list(
        range(0,coordinate_df[x_column].max().astype(int)+1, patch_length*output_um_p_px))+[ np.ceil(coordinate_df[x_column].max()).astype(int)]
    patch_delimiters_x = np.array(patch_delimiters_x)

    patch_delimiters_y = list(
        range(0,coordinate_df[y_column].max().astype(int)+1, patch_length*output_um_p_px))+[ np.ceil(coordinate_df[y_column].max()).astype(int)]
    patch_delimiters_y = np.array(patch_delimiters_y)

    hists = np.zeros((len(signatures.index), int(patch_length+kernel_bandwidth*3)+1, int(patch_length+kernel_bandwidth*3)+1))

    with tqdm.tqdm(total=(len(patch_delimiters_x)-1)*(len(patch_delimiters_y)-1)) as pbar:
        for i, x in enumerate(patch_delimiters_x[:-1]):
            for j, y in enumerate(patch_delimiters_y[:-1]):

                spatial_mask = (coordinate_df[x_column] >= x) & (coordinate_df[x_column] < (patch_delimiters_x[i+1])) & (
                    coordinate_df[y_column] >= y) & (coordinate_df[y_column] < (patch_delimiters_y[j+1]))
                ...
                df_gene_grouped = coordinates_patch.groupby(gene_column, observed=True).apply(lambda x: x[[x_column,y_column]].values)

                df_gene_grouped = df_gene_grouped[df_gene_grouped.apply(lambda x: x.shape[0]>0)]    
                size = (1+int(patch_length+kernel_bandwidth*3),)*2        
                with ThreadPoolExecutor(max_workers=n_workers) as executor:
                    fs = {executor.submit(utils.kde_2d,df_gene_grouped.loc[gene],size=size,bandwidth=kernel_bandwidth_px):gene for gene in df_gene_grouped.index}
                    for f in as_completed(fs):
                        gene = fs[f]
    ...
    if not 'ssam' in adata.uns.keys():
        adata.uns['ssam']={}

    if not 'spatial' in adata.uns.keys():
        adata.uns['spatial']={}
...
def produce_map_3d(
    ...
    coordinate_df = adata.uns['ssam']['coordinates']
    x_column,y_column,z_column,gene_column = adata.uns['ssam']['x_column'],adata.uns['ssam']['y_column'],adata.uns['ssam']['z_column'],adata.uns['ssam']['gene_column']

    out_shape = (np.ceil(coordinate_df[x_column].max()/output_um_p_px+kernel_bandwidth_px*3).astype(int),
                 np.ceil(coordinate_df[y_column].max()/output_um_p_px+kernel_bandwidth_px*3).astype(int),
                 np.ceil(coordinate_df[z_column].max()/output_um_p_px+kernel_bandwidth_px*3).astype(int))
    ...
    with tqdm.tqdm(total=(len(patch_delimiters_x)-1)*(len(patch_delimiters_y)-1)) as pbar:
        for i, x in enumerate(patch_delimiters_x[:-1]):
            for j, y in enumerate(patch_delimiters_y[:-1]):
                ...
                hists = np.zeros((len(signatures.index), int(patch_length+kernel_bandwidth*3)+1, int(patch_length+kernel_bandwidth*3)+1, int((out_shape[-1]))))
                ...
                df_gene_grouped = df_gene_grouped.apply(lambda x: x[[x_column,y_column,z_column]].values)

                df_gene_grouped = df_gene_grouped[df_gene_grouped.apply(lambda x: x.shape[0]>0)]    
                size = (1+int(patch_length+kernel_bandwidth*3),)*2 + (out_shape[-1],)   

                # print('starting threadpool')     
                with ThreadPoolExecutor(max_workers=n_workers) as executor:
                    fs = {executor.submit(utils.kde_3d,df_gene_grouped.loc[gene],size=size,bandwidth=kernel_bandwidth_px):gene for gene in genes_sorted}
                ...
                # print(corrs.min())
                corrs_winners = corrs.argmax(1)
                corrs_winners[corrs.max(1)
                                < threshold_cor] = background_value
                local_ct_map[mask] = corrs_winners
...
def count_cells_at_localmax(adata, step=1, radius=50):
    ...
    celltype_count_at_localmax = pd.DataFrame(index=adata.obs_names,columns=np.arange(ct_map.max()))
    celltype_count_at_localmax.loc[:]=0
    celltype_count_at_localmax = celltype_count_at_localmax.astype(int)

    celltype_count_at_localmax

    for i,celltype in enumerate(np.arange(ct_map.max())):

        count_aggregated_map = fftconvolve((ct_map==celltype).astype(int),mask,mode='same')
        celltype_count_at_localmax.iloc[:,i] = count_aggregated_map[(adata.obsm['spatial'][:,0]/2.5).astype(int),(adata.obsm['spatial'][:,1]/2.5).astype(int)].round().astype(int)
