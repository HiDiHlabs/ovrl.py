...
import tqdm
from ovrlp.ssam2.ssam import ssam
from ovrlp.ssam2.ssam import utils as ssam_utils
...
def transform_embeddings(expression,pca,embedder_2d,embedder_3d,colors_min_max=[None,None]):
...
def plot_embeddings(embedding,embedding_color,celltype_centers,celltypes,rasterized=False,ax=None,scatter_kwargs={'alpha':0.1,'marker':'.'},):
    colors = embedding_color.copy()#np.clip(embedding_color.copy(),0,1)

    if ax is None:
        ax = plt.gca()
        
    if not 'alpha' in scatter_kwargs:
        alpha = 0.1
    else:
    ...
    ax.scatter(embedding[:,0],embedding[:,1],c=(colors),alpha=alpha,marker=marker,rasterized=rasterized,**scatter_kwargs)

    text_artists = []
    for i in range(len(celltypes)):
        t = ax.text(np.nan_to_num((celltype_centers[i,0])),np.nan_to_num(celltype_centers[i,1]),celltypes[i],color='k',fontsize=12)
...
def compute_divergence_embedded(df,genes,visualizer, KDE_bandwidth, min_expression,metric='cosine_similarity',pca_divergence=0.8):

    signal = create_histogram(df, genes ,x_max=df.x_pixel.max(),y_max=df.y_pixel.max(),KDE_bandwidth=KDE_bandwidth)   

    genes = visualizer.genes    

    pca = PCA(pca_divergence)

    facs = pca.fit_transform(visualizer.localmax_celltyping_samples.T)
    ...
    for i,g in tqdm.tqdm(enumerate(visualizer.genes)):
        ...
        hist_top = np.dot(create_histogram(df_top, genes=[g],x_max=df.x_pixel.max(),y_max=df.y_pixel.max(),KDE_bandwidth=1.0)[mask][:,None], pca.components_[None,:,i])
        hist_bot = np.dot(create_histogram(df_bot, genes=[g],x_max=df.x_pixel.max(),y_max=df.y_pixel.max(),KDE_bandwidth=1.0)[mask][:,None], pca.components_[None,:,i])
    ...
    if metric == 'cosine_similarity':
        ...
        for i in range(hists_top_p.shape[0]):
            kl_divergence[i] = (hists_top_p[i]*np.nan_to_num(np.log(hists_top_p[i]/hists_bot_p[i]))).sum()+(hists_bot_p[i]*np.nan_to_num(np.log(hists_bot_p[i]/hists_top_p[i]))).sum()
        ...
        def pearson_cross_correlation(a,b):
            ...
            for i in range(a.shape[0]):
                out[i] = np.dot(a[i],b[i])/(np.linalg.norm(a[i])*np.linalg.norm(b[i]))
...
def compute_embedding_vectors(subset_df,signal_mask,factor):
    ...
    if len(subset_top)==0:
        ...
        signal_top = ssam_utils.kde_2d(subset_top[:,:2],size=signal_mask.shape)[signal_mask]
        signal_top = signal_top[:,None]*factor[None]
    if len(subset_bottom)==0:
        signal_bottom = 0
    else:
        signal_bottom = ssam_utils.kde_2d(subset_bottom[:,:2],size=signal_mask.shape)[signal_mask]
...
def compute_divergence_patched(
    ...
    signal_mask = signal>min_expression

    norm_top = np.zeros_like(signal)
    norm_bottom = np.zeros_like(signal)
    ...
    with tqdm.tqdm(total=(len(x_patches)-1)*(len(y_patches)-1)) as pbar:
        for i in range(len(x_patches)-1):
            for j in range(len(y_patches)-1):
                ...
                patch_df = df[(df.x>=x_) & (df.x<_x) & (df.y>=y_) & (df.y<_y)].copy()
                ...
                patch_embedding_bottom = np.zeros((patch_signal_mask.sum(),n_components))   
                
                df_gene_grouped = patch_df.groupby('gene',observed=False).apply(lambda x: x[['x','y','z','z_delim']].values)
                ...
                with ThreadPoolExecutor(max_workers=n_workers) as executor:
                    ...
                    fs = {executor.submit(compute_embedding_vectors,df_gene_grouped.loc[gene],patch_signal_mask,pca_component_matrix[:,i]):gene for i,gene in enumerate(gene_list)}
                ...
                spatial_patch_cosine_similarity[patch_signal_mask] = patch_cosine_similarity
                spatial_patch_cosine_similarity = spatial_patch_cosine_similarity[patch_padding:patch_padding+patch_size_x,patch_padding:patch_padding+patch_size_y] #remove patch edges

                cosine_similarity[x_+patch_padding:x_+patch_padding+spatial_patch_cosine_similarity.shape[0],y_+patch_padding:y_+patch_padding+spatial_patch_cosine_similarity.shape[1]] = spatial_patch_cosine_similarity
