...
def plot_signal_integrity(integrity,signal,signal_threshold=2.0,cmap='BIH',plot_hist=True):
    """
    ...
    with plt.style.context('dark_background'):
        if cmap == 'BIH':
            cmap = LinearSegmentedColormap.from_list('BIH', ['#430541','mediumvioletred','violet','powderblue','powderblue','white','white'][::-1])
            
        if plot_hist:
            fig, ax = plt.subplots(1,2,figsize=(20,figure_height),gridspec_kw={'width_ratios': [6, 1]})
        else:
            fig, ax = plt.subplots(1,1,figsize=(20,figure_height))
            ax = [ax]
            
        img = ax[0].imshow(integrity,cmap=cmap,alpha=((signal/signal_threshold).clip(0,1)),vmin=0,vmax=1)
        ax[0].invert_yaxis()
        ax[0].set_axis_off()
        
        if plot_hist:
            vals,bins = np.histogram(integrity[signal>signal_threshold], bins=50, range=(0,1),density=True)
...
def detect_doublets(coherence,signal,min_distance=10,max_incoherence=0.4,signal_cutoff=3,coherence_sigma=None):
    """
    ...
    dist_x,dist_y,dist_t = determine_localmax((1-coherence)*(signal>signal_cutoff), min_distance=min_distance,min_expression=max_incoherence)

    arg_idcs = np.argsort(dist_t)[::-1]
    dist_x,dist_y = dist_x[arg_idcs], dist_y[arg_idcs]
    
    doublet_df = pd.DataFrame({'x':dist_y,'y':dist_x,'integrity':dist_t,'signal':signal[dist_x,dist_y]})
...
class Visualizer():
    ...
    def plot_instance(self,subsample, subsample_embedding, subsample_embedding_color, x,y,window_size=30,rasterized=True):
        ...
        ax2.scatter(self.embedding[:,0],self.embedding[:,1],c=self.colors,alpha=0.5,marker='.',s=1,rasterized=rasterized)
        ax2.set_axis_off()
...
def compute_coherence_map(df, KDE_bandwidth=2,
                         min_expression=2, min_distance=8,
                         n_components_pca=0.7,
